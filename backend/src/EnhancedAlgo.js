...
// Function to evaluate the fitness of a schedule// Function to evaluate the fitness of a schedule
function evaluateFitness(schedule, labData, facultyData) {
    ...
    schedule.forEach((exam) => {
        const room = exam.RoomId;
        const examCapacity = exam['Total students'];
        const lab = labData.find((lab) => lab['Lab No'] === room);
        if (lab && examCapacity > lab.Capacity) {
