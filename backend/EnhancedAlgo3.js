...
function readDataFromExcel(filePath) {
    ...
    // Fetch exam atoms from the "combined" sheet
...
function generateRandomSchedule(data) {
    ...
    const combinedData = data.combinedData.filter(section => Object.keys(section).some(key => section[key] !== '__EMPTY'));
    const facultyData = data.facultyData;
    const labData = data.labData;
    const examAtoms = data.examAtoms;
    const timeSlotAtoms = data.timeSlotAtoms;
    ...
    shuffledData.forEach((section, index) => {
        ...
        if (section['Date'] && !usedSections.has(section['Sr No'])) {
            ...
            // Randomly select a time slot atom
            ...
            if (lab) {
                ...
                // Fetch exam atom based on section index
                const examAtom = examAtoms.find(atom => atom.id === section['Sr No']);
                ...
                const exam = {
                    ...
                    ExamName: examAtom ? examAtom.name : 'Unknown', // Include exam name
...
// Function to get a random atom from an array
...
function runGeneticAlgorithm(data) {
    ...
    bestSchedule.forEach(exam => {
        console.log(`Exam ID: ${exam.ExamId}, Section ID: ${exam.SectionId}, Subject Code: ${exam.SubjectCode}, Start Time: ${exam.StartTime}, End Time: ${exam.EndTime}, Room ID: ${exam.RoomId}, Teacher ID: ${exam.TeacherId}, Exam Name: ${exam.ExamName}`);
    });
